    ...
    try:
        ...
        for chunk in response:
            ...
            if first_token_time is None:
                ...
                print(f"\n⚡ 首字延迟：{first_token_time - start_time:.3f}s")
        ...
        if final_answer:
            ...
            with open("LLM_response.txt", "a", encoding="utf-8") as f:
                ...
                f.write(f"💡 参考答案 [{total_time:.2f}s]:\n{final_answer}\n")
                f.write("=" * 60 + "\n\n")

            print(f"\n✅ 问题 #{question_number} 解答完成 (耗时：{total_time:.2f}s)")
        else:
            print(f"\n⚠️ 未收到任何回复内容")
...
def question_detector(zhipu_api_key):
    ...
    while is_running:
        ...
        # ========== 第一分支：带问号的问题检测（Qwen3 极速版） ==========
        if stripped_text.endswith(("?", "？")):
            print(f"\n🔎 检测到带问号的句子，正在校验是否为面试问题...")

            # 🔥 超精简 Prompt
            detect_prompt = f"是否是面试问题？只输出是或否：{context_str}"

            try:
                # 🔥 使用 ollama SDK - 非流式并关闭思考
                response = chat(
                    model='qwen3.5:4b',
                ...
                detect_result = response['message']['content'].strip()

                # 严格校验，只要包含"是"就通过
                if "是" in detect_result:
        ...
        # ========== 第二分支：隐含问题检测（Qwen3 极速版） ==========
        elif len(context_buffer) >= LLM_BUFFER_SIZE and not is_valid_question:
            print(f"\n🔎 上下文缓冲区已满，检测是否有隐含面试问题...")

            # 🔥 超精简 Prompt
            detect_prompt = f"根据内容判断当前语句是否是面试问题？排除打招呼，寒暄和聊天等内容，判断问题是否为专业知识问题，只输出是或否。当前语句：{context_str}"
